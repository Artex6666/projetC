...
static const char* CHEMIN_POLICE = "./assets/fonts/Roboto_Condensed-Regular.ttf";
...
int main(int argc, char* argv[]) {
    if (argc > 1 && strcmp(argv[1], "--sdl") == 0) {
        ...
        printf("La longueur du mot est de \033[5m%d\033[0m lettres\n", length);
        ...
        if (!is_valid_word(guess)) {
            ...
            if (strlen(guess) != (size_t)length) {
                printf("Le mot propose n'a pas la meme longueur.\n");
                ++essais;
            } else {
                ...
                char valid_word = check_word(guess, secret_word, result);
                ...
                free(result);
                if(valid_word == 1){
                    printf("Vous avez trouver le bon mot qui etait : \033[32m%s\033[0m bravo !", guess);
                    return EXIT_SUCCESS;
                }
            }
        }
        --essais;
        printf("\n\nIls vous restent %hd essais\n",essais);
        }
        printf("Vous n'avez pas trouver le bon mot qui etait : \033[32m%s\033[0m", secret_word);
        free(secret_word);
        return EXIT_SUCCESS;
    }
}